...
setwd("/Users/ehickman/Library/CloudStorage/OneDrive-UniversityofNorthCarolinaatChapelHill/Rager_Lab/Projects/2023/BEE_Wristband_Project/4_Analyses/1_CurrentAnalyses/3_ChemicalDemoSSAnalysis")
```
...
Prior to analyzing the data, race will be collapsed into white and non-white, and we will join chemical data to demographic data. 
...
for (i in 1:length(endpoints)) {
  ...
  res <- wilcox.test(as.formula(paste0(endpoint, "~ race_forstats", sep = "")), 
                     data = chem_demo)
  ...
  race_wilcoxres <- rbind(race_wilcoxres, res_df)
}

# Clean data frame
race_wilcoxres <- race_wilcoxres %>%
  relocate(class_chemical, .before = pval) %>%
  mutate(padj = p.adjust(pval, method = "BH")) %>%
  rename("pval_race" = "pval", "padj_race" = "padj")

# How many chemicals with padj < 0.1?
nrow(race_wilcoxres %>% filter(padj_race < 0.1))
```
...
univar_stats_res <- list(age_correlation_spearman_cleaned, part_correlation_spearman_cleaned, 
                         race_wilcoxres, latino_wilcoxres, edu_kruskal_res)

# Combine data frames 
univar_stats_res <- univar_stats_res %>%
  reduce(full_join, by = "class_chemical")
...
race_chems <- race_wilcoxres %>% filter(padj_race < 0.1) %>% pull(class_chemical)

# Prepare p-values 
race_chems_pvals <- race_wilcoxres %>%
  filter(padj_race < 0.1) %>%
  mutate(padj_race = ifelse(padj_race > 0.001, round(padj_race, digits = 3), format(padj_race, digits = 3))) %>%
  mutate(padj_race = paste("p = ", padj_race, sep = ""))
...
race_chems_heights <- chem_demo %>%
  select(all_of(race_chems)) %>%
...
race_chems_pval_labels <- left_join(race_chems_pvals, race_chems_heights, by = "class_chemical") %>%
  separate(class_chemical, into = c(NA, "chemical"))

# Prepare data frame for graphing
chem_demo_racegraphing <- chem_demo %>%
  select(c(all_of(race_chems), race_forstats, S_ID)) %>%
  pivot_longer(-c(race_forstats, S_ID), names_to = "class_chemical", values_to = "Value") %>%
  left_join(chemical_key, by = "class_chemical") %>%
  mutate(chemical = gsub("cypermethrin", "Cypermethrin", chemical)) %>%
  mutate(race_forstats = fct_relevel(factor(race_forstats), c("White", "Non-White"))) 

chem_demo_race_graph <- ggplot() +
  geom_boxplot(chem_demo_racegraphing, 
               mapping = aes(x = race_forstats, y = Value, color = race_forstats), outlier.shape = NA) +
  geom_jitter(chem_demo_racegraphing, 
              mapping = aes(x = race_forstats, y = Value, color = race_forstats), size = 0.75, width = 0.1, height = 0.1) +
  ...
  geom_text(data = race_chems_pval_labels, mapping = aes(x = 1.5, y = pval_y, label = padj_race)) +
  facet_wrap(~chemical, nrow = 2, scales = "free_y")

chem_demo_race_graph

pdf("3_OutputFigs/Chemical_Race_SigPanel.pdf",
    width = 8, height = 4.5)
chem_demo_race_graph
invisible(dev.off())
```
...
ANCOVAres_chem = data.frame("Variable" = c("(Intercept)", "mat_age_birth", "race_forstats", "pc_latino_hispanic", "pc_ed", "Residuals"))

# Perform ANCOVA over all columns
for (i in 1:length(endpoints)) {
  
  fit = aov(as.formula(paste0(names(chem_demo_forancova)[i], "~ mat_age_birth + race_forstats + pc_latino_hispanic +  pc_ed", sep = "")), chem_demo_forancova)
...
# Save results
write.xlsx(ANCOVAres_cleaned, "2_OutputTables/ANCOVA_Res.xlsx")
```
